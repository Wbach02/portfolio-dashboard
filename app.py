...
if 'portfolio' not in st.session_state:
    st.session_state.portfolio = pd.DataFrame(columns=["Security Name", "Type", "Sector", "Yield", "Ticker", "Amount", "Total Cost", "Purchase Date", "Benchmark", "Lots"])
else:
    ...
    if "Lots" not in st.session_state.portfolio.columns:
        st.session_state.portfolio.insert(7, "Lots", pd.Series([[] for _ in range(len(st.session_state.portfolio))]))
...
def fetch_risk_metrics(ticker, benchmark, start_date, lots=None):
    try:
        ...
        if lots and len(lots) > 0:
            ...
            tot_cost = 0.0
            for lot in lots:
                l_cost = lot.get('Total Cost', 0)
                l_date = pd.to_datetime(lot.get('Purchase Date'))
                
                t_lot_data = t_data.loc[t_data.index >= l_date]
                b_lot_data = b_data.loc[b_data.index >= l_date]
                
                if not t_lot_data.empty and not b_lot_data.empty:
                    l_t_ret = (t_lot_data.iloc[-1] - t_lot_data.iloc[0]) / t_lot_data.iloc[0]
                    l_b_ret = (b_lot_data.iloc[-1] - b_lot_data.iloc[0]) / b_lot_data.iloc[0]
                    t_ret_w_sum += l_cost * l_t_ret
                    b_ret_w_sum += l_cost * l_b_ret
                    tot_cost += l_cost
            
            if tot_cost > 0:
                t_ret_total = t_ret_w_sum / tot_cost
                b_ret_total = b_ret_w_sum / tot_cost
            else:
